...
if len(sys.argv) < 5:
    print("text_shaper.py font.ttf map_out.json zero_width_out.json code_out_dir/")
    sys.exit(0)

font_path = sys.argv[1]
map_path = sys.argv[2]
zero_width_path = sys.argv[3]
...
def pack_lut(forms):
    ...
    dirtied_codepts.append(zero_width_codept)
    return lut_data, lig_data, selected_glyphs, dirtied_codepts, zero_width_codept
...
def write_lut(lut_data, lig_data, shapable_ranges, zero_width_codept, out_dir):
...
lut_data, lig_data, selected_glyphs, dirtied_codepts, zero_width_codept = pack_lut(character_forms)
...
# Write the LUTs.
write_lut(lut_data, lig_data, shapable_ranges, zero_width_codept, codegen_path)
...
json.dump({"codepoints": [zero_width_codept]}, open(zero_width_path, "w"))
